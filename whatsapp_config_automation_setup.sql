...
-- Supports: Meta Cloud API, Bulk Broadcast Engine, Message Queue,
-- Customer Stages, Customer Blocks, AI Knowledge Base, and Gemini Auto-Responder
-- =========================================================================

-- 1. session_settings: Ensure columns for WhatsApp and Google Gemini credentials
DO $$ 
BEGIN
    -- WhatsApp API Columns
    ...
    -- Google Gemini AI Studio Columns
    ...
    -- Sync back to existing column names if they exist
...
-- 2. message_batches: Track bulk broadcast campaigns
...
-- 3. message_queue: Message log and queue items
...
-- Add missing columns to message_queue if existed
DO $$
BEGIN
...
-- 4. chat_messages: Unified live chat store
...
-- 5. ai_knowledge_base: Q&A repository for Gemini AI auto-responder
...
-- Seed initial Snehyoga Knowledge Base entries if empty
...
-- 6. customer_lead_stage: Track customer funnel stages
...
-- 7. customer_blocks: Excluded from all broadcasts
...
-- Row Level Security (RLS) setup allowing dashboard read & write
